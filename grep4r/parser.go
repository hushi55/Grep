...
func parseArrayReply(cn *conn, p multiBulkParser, line []byte) (interface{}, error) {
	...
	if err != nil {
		log.Error("parseArrayReply error: %s", err)
		return nil, err
	} 
...
func readScanStatusReply(cn *conn) ([]string, int64, error) {
	n, err := readArrayHeader(cn)
	if err != nil {
		return nil, 0, err
	}
...
func sliceParser(cn *conn, n int64) (interface{}, error) {
	...
	for i := int64(0); i < n; i++ {
		vals[i], err = readReply(cn, sliceParser)
		if err == Nil {
			log.Warn("append redis nil")
			vals[i] = nil
		} else if err != nil {
			log.Error("sliceParser error : %s", err)
			return nil, err
		} else {
